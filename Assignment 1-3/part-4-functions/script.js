...
d3.csv('../../data/world-bank/cb704cf4-fe8f-4f1b-ae0c-2a51c26f295f_Data.csv', parse)
	...
	.then(function(rows){
		...
		const group = [];
		rows.forEach(d => {
			if(d.countryCode === country && d.series === series){
				group.push(d);			
			}
		});
		rows.sort((a, b) => {
		...
		//I am confused by the question. Normally function has its scope, which means it cannot be moved without a consideration.
	});
...
function transform(data){
	return data.map(function(d){
		...
		//this is destructing declaration 
		//equals to const countryName = d.countryName; const countryCode = d.countryCode; const series = d.series;
